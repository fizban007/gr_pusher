...
struct Kerr_Schild {
  ...
  Kerr_Schild() {}
  Kerr_Schild(double rg, double a) : rg_(rg), a_(a) {}
...
Gamma(const Vec3<Data>& x, const Vec3<Data>& u,
  ...
  Data b = 2.0 * (metric.g01(x[0], x[1], x[2]) * u[0] +
                  metric.g02(x[0], x[1], x[2]) * u[1] +
...
u0_energy(const Vec3<Data>& x, const Vec3<Data>& u,
  ...
  result += metric.g01(x[0], x[1], x[2]) * u[0] +
            metric.g02(x[0], x[1], x[2]) * u[1] +
            metric.g03(x[0], x[1], x[2]) * u[2];
  return result;
}
...
Fu(int n, const Vec3<double>& u0, const Vec3<double>& x0, const Vec3<Data>& u,
   const Vec3<Data>& x, const Kerr_Schild<Data>& metric, double dt) {
  ...
  if (n == 0) {
    result -=
        0.5 * CONNECTION(metric, 1, 0, 0, mid_x) * gamma +
    ...
    result -=
        0.5 * CONNECTION(metric, 2, 0, 0, mid_x) * gamma +
    ...
    result -=
        0.5 * CONNECTION(metric, 3, 0, 0, mid_x) * gamma +
...
Fx(int n, const Vec3<double>& u0, const Vec3<double>& x0, const Vec3<Data>& u,
  ...
  Data gamma = Gamma(mid_x, mid_u, metric);
  if (n == 0) {
    return x[0] - x0[0] - dt * u[0] / gamma;
  } else if (n == 1) {
    return x[1] - x0[1] - dt * u[1] / gamma;
  } else if (n == 2) {
    return x[2] - x0[2] - dt * u[2] / gamma;
  } else {
    return 0.0;
  }
}
...
main(int argc, char* argv[]) {
  ...
  Kerr_Schild<var> metric(1.0, 0.0);
  Kerr_Schild<double> metric_num(1.0, 0.0);
  const double dt = 0.1;
