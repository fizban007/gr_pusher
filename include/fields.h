...
class FieldBase {
 public:
  ...
  /// grid object.
  FieldBase(const Grid &grid) : m_grid(&grid), m_grid_size(grid.size()) {}
...
class ScalarField : public FieldBase {
 public:
  ...
  // template <int Order>
  T interpolate(const Vec3<int> &c, const Vec3<Pos_t> &rel_pos, int order = 1) const;
...
#include "detail/fields_impl.hpp"
