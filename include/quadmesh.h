...
// This allows declaring a quadmesh in __constant__ memory.
  ...
  HD_INLINE int reduced_dim(int i) const { return (dims[i] - 2 * guard[i]); }
  ...
  HD_INLINE Scalar pos(int i, int n, int stagger) const {
    return pos(i, n, stagger * 0.5 + 0.5);
  }
...
#endif
  // HD_INLINE Scalar pos(int i, int n, StaggerType stagger,
  ...
  ////////////////////////////////////////////////////////////////////////////////
  HD_INLINE int find_cell(const Vec3<Scalar>& pos, Vec3<float>& rel_pos) const {
